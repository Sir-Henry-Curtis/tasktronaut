...
Auto Approve is evaluated per tool call. When Cline is about to read a file, edit a file, run a command, or fetch external URL content, it checks your Auto Approve settings for that category.
...
| Fetch external URLs | Built-in external URL fetching (`web_fetch`) |
...
- Leave edits, commands, external URL fetching, and MCP off until you have a specific reason
...
YOLO mode is Auto Approve on steroids. Check the box and Cline auto-approves everything: file changes, terminal commands, external URL fetching, MCP tools, and mode transitions.
