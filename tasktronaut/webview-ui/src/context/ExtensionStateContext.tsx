...
export const ExtensionStateContextProvider: React.FC<{
	...
	const [showGsd, setShowGsd] = useState(false)
	...
	const hideGsd = useCallback(() => setShowGsd(false), [setShowGsd])
	...
	const navigateToMcp = useCallback(
		...
		[setShowMcp, setMcpTab, setShowSettings, setShowHistory, setShowAccount, setShowWorktrees, setShowGsd],
	)
	...
	const navigateToHistory = useCallback(() => {
		...
		setShowGsd(false)
		setShowHistory(true)
	}, [setShowSettings, closeMcpView, setShowAccount, setShowWorktrees, setShowGsd, setShowHistory])
	...
	const navigateToAccount = useCallback(() => {
		...
		setShowGsd(false)
		setShowAccount(true)
	}, [setShowSettings, closeMcpView, setShowHistory, setShowWorktrees, setShowGsd, setShowAccount])
	...
	}, [setShowSettings, closeMcpView, setShowHistory, setShowAccount, setShowGsd, setShowWorktrees])

	const navigateToGsd = useCallback(() => {
		setShowSettings(false)
		closeMcpView()
		setShowHistory(false)
		setShowAccount(false)
		setShowWorktrees(false)
		setShowGsd(true)
	}, [setShowSettings, closeMcpView, setShowHistory, setShowAccount, setShowWorktrees, setShowGsd])
	...
	const gsdButtonClickedSubscriptionRef = useRef<(() => void) | null>(null)
	...
	useEffect(() => {
		...
		// Set up GSD button clicked subscription
		gsdButtonClickedSubscriptionRef.current = UiServiceClient.subscribeToGsdButtonClicked(EmptyRequest.create({}), {
			onResponse: () => {
				navigateToGsd()
			},
			onError: (error) => {
				console.error("Error in GSD button clicked subscription:", error)
			},
			onComplete: () => {
				console.log("GSD button clicked subscription completed")
			},
		})
		...
		return () => {
			...
			if (gsdButtonClickedSubscriptionRef.current) {
				gsdButtonClickedSubscriptionRef.current()
				gsdButtonClickedSubscriptionRef.current = null
			}
