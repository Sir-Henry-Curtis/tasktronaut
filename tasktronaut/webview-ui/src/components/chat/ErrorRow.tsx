		...
		switch (errorType) {
			...
			case "mistake_limit_reached":
				...
				if (rawApiError) {
					...
					// FORK MOD: Balance/SpendLimit error components removed (account system disabled).
					if (
					...
					return (
						<p className="m-0 whitespace-pre-wrap text-error wrap-anywhere flex flex-col gap-3">
							...
							{errorMessage?.toLowerCase()?.includes("powershell") && (
								<div>
									It looks like Windows PowerShell is unavailable in this environment. Check that PowerShell is
									installed and available on your PATH, then retry.
								</div>
							)}
			...
			case "clineignore_error":
				return (
					<div className="flex flex-col p-2 rounded text-xs opacity-80 bg-quote text-foreground">
						<div>
							Tasktronaut tried to access <code>{message.text}</code> which is blocked by the project's ignore
							rules.
						</div>
					</div>
				)
