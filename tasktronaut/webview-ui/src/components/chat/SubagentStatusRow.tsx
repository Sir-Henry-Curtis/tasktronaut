...
const formatExecutionMetadata = (entry: SubagentStatusItem): string[] => {
	...
	const lines = summaryParts.length > 0 ? [summaryParts.join(" · ")] : []
...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	const title = singular ? "Tasktronaut wants to use a subagent:" : "Tasktronaut wants to use subagents:"
		...
		<div className="mb-2">
			...
			<div className="space-y-2">
				{data.items.map((entry, index) => {
					...
					const executionLines = formatExecutionMetadata(entry)
					return (
						<div
							...
							{shouldShowStats && executionLines.length > 0 && (
								<div className="mt-1 text-[10px] opacity-65 min-w-0 whitespace-pre-wrap break-words font-mono">
									{executionLines.join("\n")}
								</div>
							)}
