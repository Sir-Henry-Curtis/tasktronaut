	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Tasktronaut Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Tasktronaut Rules & Workflows" : "Show Tasktronaut Rules & Workflows"}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Tasktronaut with system-level guidance. Think of them as a
									persistent way to include context and preferences for your projects or globally for every
									conversation.
								</p>
							) : currentView === "workflows" ? (
								<p>
									Workflows allow you to define a series of steps to guide Tasktronaut through a repetitive set
								...
								<p>
									Skills are reusable instruction sets that Tasktronaut can activate on-demand. When a task
									matches a skill's description, Tasktronaut uses the <span className="font-bold">use_skill</span>{" "}
									tool to load the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Tasktronaut's execution
								...
								<div className="text-xs text-description mb-4">
									<p>
										{isWindows
											...
											: "Toggle to enable or disable a hook with chmod +x or chmod -x."}
									</p>
								</div>
								{/* Hooks Tab */}
