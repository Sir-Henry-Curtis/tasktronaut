...
/**
 * Apply task-related options (mode, model, thinking) to StateManager.
	...
	const shutdown = async (signal: string) => {
		...
		try {
			if (activeContext) {
				...
				// Best-effort flush of pending state when no active context
				try {
...
async function runTask(prompt: string, options: TaskOptions & { images?: string[] }, existingContext?: CliContext) {
	...
	// Apply shared task options (mode, model, thinking)
...
program
	...
	.option("--max-consecutive-mistakes <count>", "Maximum consecutive mistakes before halting the task")
...
mcpCommand
	.command("add")
	.description("Add an MCP server shortcut to tasktronaut_mcp_settings.json")
...
program
	...
	.option("-v, --verbose", "Show verbose output")
	.action((options) => checkForUpdates(CLI_VERSION, { verbose: options.verbose }))
...
async function showWelcome(options: TaskOptions) {
	...
	// --auto-approve-all affects the initial TUI state.
	applyTaskOptions(options)
	await StateManager.get().flushPendingState()
...
program
	...
	.action(async (prompt, options) => {
		if (options.update) {
			if (prompt || options.taskId || options.continue || options.acp) {
				printWarning("Use --update without a prompt or task flags.")
				exit(1)
			}

			await checkForUpdates(CLI_VERSION, { verbose: options.verbose })
			return
		}
