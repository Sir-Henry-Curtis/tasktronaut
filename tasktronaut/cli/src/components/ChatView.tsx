...
export const ChatView: React.FC<ChatViewProps> = ({
	...
	// Sync mode from core state updates
	useEffect(() => {
	...
	useInput((input, key) => {
		...
		if (
			buttonConfig.enableButtons && !isSpinnerActive && textInput === ""
		) {
	...
	let inputPrompt = ""
	if (pendingAsk && askType === "options" && askOptions.length > 0) {
	...
	return (
		<Box flexDirection="column" key={taskSwitchKey} width="100%">
			...
			<Box flexDirection="column" width="100%">
				...
				{buttonConfig.enableButtons && !isSpinnerActive && <ActionButtons config={buttonConfig} mode={mode} />}
