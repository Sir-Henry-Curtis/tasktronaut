...
async function readMcpSettings(configDir: string): Promise<McpSettingsFile> {
	const settingsPath = path.join(configDir, "data", "settings", "tasktronaut_mcp_settings.json")
...
describe("addMcpServerShortcut", () => {
	it("writes stdio servers with type=stdio", async () => {
		...
		await addMcpServerShortcut("localboard", ["localboard", "mcp"], { config: configDir })
		const settings = await readMcpSettings(configDir)

		expect(settings.mcpServers.localboard).toEqual({
			command: "localboard",
			args: ["mcp"],
			type: "stdio",
		})
