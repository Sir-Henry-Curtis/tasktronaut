...
export async function checkForUpdates(currentVersion: string, options: CheckForUpdatesOptions = {}) {
	printInfo("Checking for Tasktronaut updates...")
	...
	try {
		...
		if (!canUpdateCline) {
			if (clineIsUpToDate) {
				printInfo(`You are already on the latest version tasktronaut@${currentVersion}`)
			}
			exit(0)
		}
		...
		if (canUpdateCline && updateCommand && latestClineVersion) {
			printInfo(`Installing tasktronaut@${latestClineVersion}...`)
			try {
				const clineUpdateCode = await runClineUpdate(updateCommand)
				if (clineUpdateCode === 0) {
					installedUpdates.push(`tasktronaut@${latestClineVersion}`)
				} else {
					printWarning(`Tasktronaut update failed. Please try running: ${updateCommand}`)
					hadFailure = true
				}
			} catch (error) {
				const message = error instanceof Error ? error.message : String(error)
				printWarning(`Failed to run Tasktronaut update: ${message}`)
