...
export class ClineToolSet {
	...
	private static getDynamicSubagentToolSpecs(variant: PromptVariant, context: SystemPromptContext): ClineToolSpec[] {
		...
		return agentConfigs.map(({ toolName, config }) => ({
			...
			name: toolName,
			description: `Use the "${config.name}" subagent in parallel. Each prompt launches one focused run of the same agent type and returns aggregated results with tool and token stats.`,
			contextRequirements: (ctx) => ctx.subagentsEnabled === true && !ctx.isSubagentRun,
			parameters: [
				{
					name: "prompt_1",
					required: true,
					instruction: `First instruction for the "${config.name}" agent.`,
				},
				{
					name: "prompt_2",
					required: false,
					instruction: `Optional second instruction for the "${config.name}" agent.`,
				},
				{
					name: "prompt_3",
					required: false,
					instruction: `Optional third instruction for the "${config.name}" agent.`,
				},
				{
					name: "prompt_4",
					required: false,
					instruction: `Optional fourth instruction for the "${config.name}" agent.`,
				},
				{
					name: "prompt_5",
					required: false,
					instruction: `Optional fifth instruction for the "${config.name}" agent.`,
				},
			],
		}))
	}
