...
export const GlobalFileNames = {
	...
	mcpSettings: "tasktronaut_mcp_settings.json",
	clineRules: ".tasktronautrules",
	workflows: ".tasktronautrules/workflows",
	hooksDir: ".tasktronautrules/hooks",
	clineruleSkillsDir: ".tasktronautrules/skills",
	clineSkillsDir: ".tasktronautrules/skills",
...
/**
 * Returns the cross-platform path to the Tasktronaut home directory (~/.tasktronaut).
 * This works on macOS, Linux, and Windows:
 * - macOS: /Users/username/.tasktronaut
 * - Linux: /home/username/.tasktronaut
 * - Windows: C:\Users\username\.tasktronaut
 */
export function getClineHomePath(): string {
	return path.join(os.homedir(), ".tasktronaut")
}
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const clineRulesDir = path.join(userDocumentsPath, "Tasktronaut", "Rules")
	try {
		await fs.mkdir(clineRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Tasktronaut", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
...
export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const clineWorkflowsDir = path.join(userDocumentsPath, "Tasktronaut", "Workflows")
	try {
		await fs.mkdir(clineWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Tasktronaut", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
...
export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Tasktronaut", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Tasktronaut", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const clineHooksDir = path.join(userDocumentsPath, "Tasktronaut", "Hooks")
	try {
		await fs.mkdir(clineHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Tasktronaut", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
 ...
 * 3. Each workspace root's .tasktronautrules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .tasktronautrules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .tasktronautrules/hooks folder in this workspace root.
