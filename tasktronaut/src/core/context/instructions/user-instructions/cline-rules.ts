	...
	if (await fileExistsAtPath(globalClineRulesFilePath)) {
		if (await isDirectory(globalClineRulesFilePath)) {
			try {
				...
				Logger.error(`Failed to read .tasktronautrules directory at ${globalClineRulesFilePath}`)
			}
		} else {
		...
		if (await isDirectory(clineRulesFilePath)) {
			try {
				const rulesFilePaths = await readDirectory(clineRulesFilePath, [
					[".tasktronautrules", "workflows"],
					[".tasktronautrules", "hooks"],
					[".tasktronautrules", "skills"],
				])
				...
				Logger.error(`Failed to read .tasktronautrules directory at ${clineRulesFilePath}`)
			}
		} else {
			try {
				if (clineRulesFilePath in toggles && toggles[clineRulesFilePath] !== false) {
					...
					if (raw) {
						// Keep single-file .tasktronautrules behavior consistent with directory/remote rules:
				...
				Logger.error(`Failed to read .tasktronautrules file at ${clineRulesFilePath}`)
			}
		}
	}
