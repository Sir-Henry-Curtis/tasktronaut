...
describe("SubagentToolHandler", () => {
	...
	it("runs configured subagent tools with multiple prompt_n parameters in parallel", async () => {
		const { config } = createConfig({ autoApproveSafe: true, autoApproveAll: true })
		const handler = new UseSubagentsToolHandler()
		...
		assert.equal(runStub.firstCall.args[0], "focus tech")
		assert.equal(runStub.secondCall.args[0], "focus arch")
	})

	it("prepares isolated worker subagents sequentially before running them", async () => {
		const { config } = createConfig({ autoApproveSafe: true, autoApproveAll: true })
		const handler = new UseSubagentsToolHandler()
		const dynamicToolName = "use_subagent_gsd_executor"
		sinon.stub(AgentConfigLoader, "getInstance").returns({
			resolveSubagentNameForTool: (toolName: string) => (toolName === dynamicToolName ? "gsd-executor" : undefined),
			getCachedConfig: () => ({
				name: "gsd-executor",
		...
		const prepareStub = sinon.stub(SubagentRunner.prototype, "prepare").callsFake(async function () {
			prepareOrder.push(prepareOrder.length + 1)
		})
		const runStub = sinon.stub(SubagentRunner.prototype, "run").resolves({
			status: "completed",
			result: "done",
		...
		sinon.assert.calledTwice(runStub)
		assert.deepEqual(prepareOrder, [1, 2])
	})

	it("includes worker execution metadata in status updates and summary output", async () => {
		const { config, callbacks } = createConfig({ autoApproveSafe: true, autoApproveAll: true })
		const handler = new UseSubagentsToolHandler()
		const dynamicToolName = "use_subagent_gsd_executor"
		sinon.stub(AgentConfigLoader, "getInstance").returns({
		...
		sinon.stub(SubagentRunner.prototype, "run").resolves({
			...
			execution: {
				...
				isolation: "worktree",
				isolated: true,
				cwd: "/repo/.tasktronaut/worktrees/agent-1",
				worktreePath: "/repo/.tasktronaut/worktrees/agent-1",
				branchName: "tasktronaut/agent-1",
			},
			stats: {
				toolCalls: 1,
				inputTokens: 2,
		...
		const result = await handler.execute(config, {
			...
			name: dynamicToolName as ClineDefaultTool,
			params: {
				prompt: "execute plan one",
			},
			partial: false,
		})

		assert.match(String(result), /Execution: role=worker · isolation=worktree · isolated=true · cwd=\/repo\/\.tasktronaut\/worktrees\/agent-1/)
		assert.match(String(result), /Branch: tasktronaut\/agent-1/)
		assert.match(String(result), /Worktree: \/repo\/\.tasktronaut\/worktrees\/agent-1/)

		const subagentStatusCalls = callbacks.say.getCalls().filter((call) => call.args[0] === "subagent")
		assert.ok(subagentStatusCalls.length >= 1)
		const finalPayload = JSON.parse(subagentStatusCalls[subagentStatusCalls.length - 1].args[1])
		assert.deepEqual(finalPayload.items[0].execution, {
			role: "worker",
			isolation: "worktree",
			isolated: true,
			cwd: "/repo/.tasktronaut/worktrees/agent-1",
	...
	it("blocks multiple prompts for worker subagents without worktree isolation", async () => {
		const { config } = createConfig({ autoApproveSafe: true, autoApproveAll: true })
		const handler = new UseSubagentsToolHandler()
		const dynamicToolName = "use_subagent_gsd_verifier"
		sinon.stub(AgentConfigLoader, "getInstance").returns({
			resolveSubagentNameForTool: (toolName: string) => (toolName === dynamicToolName ? "gsd-verifier" : undefined),
			getCachedConfig: () => ({
				name: "gsd-verifier",
	...
	it("requires prompt for configured subagent tools", async () => {
		...
		assert.equal(result, "missing")
		assert.equal(taskState.consecutiveMistakeCount, 1)
		sinon.assert.calledWithExactly(callbacks.sayAndCreateMissingParamError, ClineDefaultTool.USE_SUBAGENTS, "prompt_1")
	})
})
