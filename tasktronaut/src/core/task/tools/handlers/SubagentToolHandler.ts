...
function resolveConfiguredSubagentConfig(toolName: string) {
	const loader = AgentConfigLoader.getInstance()
	const subagentName = loader.resolveSubagentNameForTool(toolName)
	return subagentName ? loader.getCachedConfig(subagentName) : undefined
}

function collectPrompts(block: ToolUse, configuredSubagentName?: string): string[] {
	if (configuredSubagentName) {
		const batchPrompts = PROMPT_KEYS.map((key) => block.params[key]?.trim()).filter((prompt): prompt is string => !!prompt)
		if (batchPrompts.length > 0) {
			return batchPrompts
		}

		const dynamicPrompt = block.params.prompt?.trim()
...
function formatExecutionSummary(entry: SubagentStatusItem): string[] {
	...
	const summaryParts = [
		...
		execution.role ? `role=${execution.role}` : undefined,
		execution.isolation ? `isolation=${execution.isolation}` : undefined,
		execution.isolated !== undefined ? `isolated=${execution.isolated}` : undefined,
		execution.cwd ? `cwd=${execution.cwd}` : undefined,
	].filter((part): part is string => Boolean(part))

	const lines = summaryParts.length > 0 ? [`Execution: ${summaryParts.join(" · ")}`] : []
	if (execution.branchName) {
		lines.push(`Branch: ${execution.branchName}`)
	}
	if (execution.worktreePath) {
		lines.push(`Worktree: ${execution.worktreePath}`)
	}
	return lines
}
...
export class UseSubagentsToolHandler implements IFullyManagedTool {
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		const prompts = configuredSubagentName
			? (() => {
					const batchPrompts = PROMPT_KEYS.map((key) =>
					...
					return [uiHelpers.removeClosingTag(block, "prompt", block.params.prompt?.trim())?.trim()].filter(
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const configuredSubagentName = resolveConfiguredSubagentName(block.name)
		const configuredSubagentConfig = resolveConfiguredSubagentConfig(block.name)
		...
		if (prompts.length === 0) {
			...
			return await config.callbacks.sayAndCreateMissingParamError(
				this.name,
				configuredSubagentName ? "prompt_1" : "prompt_1",
			)
		}
		...
		const entries: SubagentStatusItem[] = prompts.map((prompt, index) => ({
			...
			execution: configuredSubagentConfig
				? {
						role: configuredSubagentConfig.role,
						isolation: configuredSubagentConfig.isolation,
						isolated: false,
						cwd: config.cwd,
					}
				: undefined,
		...
		settled.forEach((result, index) => {
			...
			entries[index].execution = result.value.execution
		...
		const summary = [
			...
			...entries.map((entry) => {
				...
				const executionLines = formatExecutionSummary(entry)
				const lines = [header, ...executionLines]
				if (detail) {
					lines.push(detail)
				}
				return lines.join("\n")
