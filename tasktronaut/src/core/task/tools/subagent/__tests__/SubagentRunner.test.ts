...
import * as gitWorktreeUtils from "@/utils/git-worktree"
...
describe("SubagentRunner", () => {
	...
	it("creates isolated worktrees for worker subagents and forwards the isolated cwd to commands", async () => {
		const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tasktronaut-subagent-runner-"))
		const worktreePath = path.join(workspaceRoot, ".tasktronaut", "worktrees", "agent-1")
		const createMessage = sinon.stub().callsFake(async function* () {
			yield {
				type: "tool_calls",
				tool_call: {
					function: {
						id: "toolu_subagent_worker_1",
						name: ClineDefaultTool.LIST_FILES,
						arguments: JSON.stringify({ path: ".", recursive: false }),
					},
				},
			...
			yield {
				...
				tool_call: {
					function: {
						id: "toolu_subagent_worker_complete_1",
						name: ClineDefaultTool.ATTEMPT,
						arguments: JSON.stringify({ result: "done" }),
					},
				},
		...
		sinon.stub(skills, "discoverSkills").resolves([])
		sinon.stub(skills, "getAvailableSkills").returns([])
		sinon.stub(SubagentBuilder.prototype, "getRole").returns("worker")
		sinon.stub(SubagentBuilder.prototype, "getIsolationMode").returns("worktree")
		sinon.stub(Date, "now").returns(1111)
		sinon.stub(Math, "random").returns(0.123456789)
		sinon.stub(gitWorktreeUtils, "getGitRootPath").resolves(workspaceRoot)
		sinon.stub(gitWorktreeUtils, "createWorktree").resolves({
			success: true,
			message: "ok",
			worktree: {
				path: worktreePath,
				branch: "tasktronaut/agent-1",
				commitHash: "abc123",
				isCurrent: false,
				isBare: false,
				isDetached: false,
		...
		const listFilesHandler = {
			execute: sinon.stub().callsFake(async (subagentConfig: TaskConfig) => {
				assert.equal(subagentConfig.cwd, worktreePath)
				await subagentConfig.callbacks.executeCommandTool("pwd", undefined)
				return "ok"
			}),
			getDescription: sinon.stub().returns("list_files"),
		}
		;(config.coordinator.getHandler as sinon.SinonStub).withArgs(ClineDefaultTool.LIST_FILES).returns(listFilesHandler)

		const runner = new SubagentRunner(config)
		const result = await runner.run("List files", () => {})

		assert.equal(result.status, "completed")
		assert.equal(result.execution?.role, "worker")
		assert.equal(result.execution?.isolation, "worktree")
		assert.equal(result.execution?.isolated, true)
		assert.equal(result.execution?.cwd, worktreePath)
		assert.equal(result.execution?.worktreePath, worktreePath)
		assert.equal(result.execution?.branchName, "tasktronaut/agent-1")
		assert.match(result.execution?.runId || "", /^subagent-/)
		sinon.assert.calledOnceWithExactly(gitWorktreeUtils.getGitRootPath as sinon.SinonStub, workspaceRoot)
		sinon.assert.calledOnce(gitWorktreeUtils.createWorktree as sinon.SinonStub)
		sinon.assert.calledWithMatch(
			config.callbacks.executeCommandTool as sinon.SinonStub,
			"pwd",
			undefined,
			sinon.match({ cwd: worktreePath }),
		)

		const registryPath = path.join(workspaceRoot, ".tasktronaut", "runtime", "subagent-executions.json")
		...
		assert.equal(registry.runs[0].run_id, result.execution?.runId)
		assert.equal(registry.runs[0].status, "completed")
		assert.equal(registry.runs[0].execution_cwd, worktreePath)
		assert.equal(registry.runs[0].worktree_path, worktreePath)
		assert.equal(registry.runs[0].branch_name, "tasktronaut/agent-1")
	})
