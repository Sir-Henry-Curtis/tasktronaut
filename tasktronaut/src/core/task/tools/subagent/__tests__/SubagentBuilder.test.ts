...
import { SUBAGENT_DEFAULT_ALLOWED_TOOLS, SUBAGENT_SYSTEM_SUFFIX, SUBAGENT_WORKER_SYSTEM_SUFFIX, SubagentBuilder } from "../SubagentBuilder"
...
describe("SubagentBuilder", () => {
	...
	it("uses cached config by subagent name and applies act-mode provider model override", () => {
		...
		assert.equal(builder.getRole(), "worker")
		assert.equal(builder.getIsolationMode(), "worktree")
		...
		assert.match(prompt, new RegExp(SUBAGENT_WORKER_SYSTEM_SUFFIX.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")))
	})
	...
	it("uses defaults when no cached config is provided", () => {
		...
		assert.equal(builder.getRole(), "research")
		assert.equal(builder.getIsolationMode(), "inherit")
