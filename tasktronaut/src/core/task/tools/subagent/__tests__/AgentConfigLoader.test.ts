...
Prompt body`
	...
	it("loads markdown and yaml agent configs from workspace-local .tasktronaut/agents", async () => {
		const tempHome = await createTempHomeDir()
		tempDirs.push(tempHome)
...
Prompt body`,
		...
		await fs.writeFile(
			path.join(directoryPath, "reviewer.md"),
			`---
name: reviewer
...
Reviewer prompt`,
		...
		await fs.writeFile(path.join(directoryPath, "reviewer.yml"), "not frontmatter", "utf8")
