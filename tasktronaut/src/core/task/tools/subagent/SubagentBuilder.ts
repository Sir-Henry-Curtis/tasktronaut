...
import type { AgentBaseConfig, AgentIsolationMode, AgentRole } from "./AgentConfigLoader"
...
export const SUBAGENT_WORKER_SYSTEM_SUFFIX = `\n\n# Subagent Execution Mode
You are running as an execution subagent. Your job is to complete the assigned work directly in the workspace you were given.
You may read files, modify files, run commands, and create commits when the prompt requires it.
Treat the provided workspace as your ownership boundary. If you are running inside an isolated worktree, keep all file edits and git operations inside that isolated workspace.
Do not modify shared planning or orchestration artifacts unless the prompt explicitly assigns them to you.
When you have finished the assigned work, call the attempt_completion tool.
The attempt_completion result field is sent directly to the main agent, so include the concrete outcome, any commits or artifacts created, and a section titled "Relevant file paths" listing only file paths, one per line.
`
...
export class SubagentBuilder {
	...
	getDisplayName(): string {
		return this.agentConfig.name?.trim() || "subagent"
	}

	getRole(): AgentRole {
		return this.agentConfig.role ?? "research"
	}

	getIsolationMode(): AgentIsolationMode {
		return this.agentConfig.isolation ?? "inherit"
	}
	...
	buildSystemPrompt(generatedSystemPrompt: string): string {
		...
		const executionSuffix = this.getRole() === "worker" ? SUBAGENT_WORKER_SYSTEM_SUFFIX : SUBAGENT_SYSTEM_SUFFIX
		return `${systemPrompt}${this.buildAgentIdentitySystemPrefix()}${executionSuffix}`
	}
