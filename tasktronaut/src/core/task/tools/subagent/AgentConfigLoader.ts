...
export const TASKTRONAUT_DIRECTORY_NAME = ".tasktronaut"
export const AGENTS_CONFIG_DIRECTORY_NAME = "agents"
const SUBAGENT_DYNAMIC_TOOL_NAMESPACE = "subagent"
const AgentRoleSchema = z.enum(["research", "worker"])
const AgentIsolationSchema = z.enum(["inherit", "worktree"])
...
export type AgentIsolationMode = z.infer<typeof AgentIsolationSchema>
...
export function getAgentsConfigPath(workspacePath: string): string {
	return path.join(workspacePath, TASKTRONAUT_DIRECTORY_NAME, AGENTS_CONFIG_DIRECTORY_NAME)
}
...
export async function readAgentConfigsFromDisk(workspacePath?: string): Promise<Map<string, AgentBaseConfig>> {
	const configs = new Map<string, AgentBaseConfig>()
	if (!workspacePath?.trim()) {
		return configs
	}

	const agentsDirectoryPath = getAgentsConfigPath(workspacePath)
...
export class AgentConfigLoader {
	...
	private constructor(workspacePath?: string) {
		...
		this.directoryPath = workspacePath ? getAgentsConfigPath(workspacePath) : ""
	...
	public static getInstance(workspacePath = AgentConfigLoader.workspacePath): AgentConfigLoader {
		if (!AgentConfigLoader.instance) {
			AgentConfigLoader.instance = new AgentConfigLoader(workspacePath)
		}
		return AgentConfigLoader.instance
	}

	public static async setWorkspacePath(workspacePath?: string): Promise<void> {
		const normalizedWorkspacePath = workspacePath?.trim() || undefined
		if (AgentConfigLoader.workspacePath === normalizedWorkspacePath) {
			return
		}

		AgentConfigLoader.workspacePath = normalizedWorkspacePath
		if (!AgentConfigLoader.instance) {
			return
		}
	...
	public async load(): Promise<ReadonlyMap<string, AgentBaseConfig>> {
		const configs = await readAgentConfigsFromDisk(this.workspacePath)
	...
	public async dispose(): Promise<void> {
		setDynamicToolUseNames(SUBAGENT_DYNAMIC_TOOL_NAMESPACE, [])
		if (!this.watcher) {
			return
		}
