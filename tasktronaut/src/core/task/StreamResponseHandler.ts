...
class ToolUseHandler {
	...
	getPartialToolUsesAsContent(): ToolUse[] {
		...
		for (const pending of pendingToolUses) {
			...
			// OpenAI Responses tool calls use fc_* function ids plus a later call_* id.
			// Do not execute until the real call id arrives, or tool results can no longer
			// be matched back to the originating function call on the next turn.
			if (pending.id.startsWith("fc_") && (!pending.callIdConfirmed || !pending.inputComplete)) {
				continue
			}
	...
	private createPendingToolUse(id: string, name: string, callId?: string): PendingToolUse {
		...
		const pending: PendingToolUse = {
			...
			inputComplete: false,
			jsonParser,
			// Keep a temporary local identifier until the provider supplies a real call_id.
			call_id: callId || id || nanoid(8),
			callIdConfirmed: !!callId,
			signature: undefined,
		}
