...
describe("UserPromptSubmit Hook", () => {
	...
	describe("Hook Input Format", () => {
		it("should receive prompt text from user content", async function () {
			...
			const hookPath = path.join(tempDir, ".tasktronautrules", "hooks", "UserPromptSubmit")
...
process.exit(1)`
	...
	describe("Global and Workspace Hooks", () => {
		...
		beforeEach(async () => {
			...
			workspaceHooksDir = path.join(tempDir, ".tasktronautrules", "hooks")
...
console.log(JSON.stringify({
  ...
  contextModification: "GLOBAL: Prompt received"
			...
			const workspaceHookPath = path.join(tempDir, ".tasktronautrules", "hooks", "UserPromptSubmit")
