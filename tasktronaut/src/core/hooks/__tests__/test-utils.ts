 ...
 *
 * @param baseDir Base directory where .tasktronautrules/hooks will be created
 * @returns Path to the created hooks directory
 *
 * @example
 * const hooksDir = await createHooksDirectory("/tmp/test")
 * // Returns: "/tmp/test/.tasktronautrules/hooks"
 */
export async function createHooksDirectory(baseDir: string): Promise<string> {
	const hooksDir = path.join(baseDir, ".tasktronautrules", "hooks")
...
export async function writeHookScriptForPlatform(hookPath: string, nodeScript: string): Promise<void> {
	const canonicalHookPath = hookPath.replace(
		`${path.sep}.clinerules${path.sep}hooks${path.sep}`,
		`${path.sep}.tasktronautrules${path.sep}hooks${path.sep}`,
	)
	const targetHookPaths = canonicalHookPath === hookPath ? [hookPath] : [hookPath, canonicalHookPath]

	if (process.platform === "win32") {
		for (const targetHookPath of targetHookPaths) {
			await fs.mkdir(path.dirname(targetHookPath), { recursive: true })
			const jsPath = `${targetHookPath}.js`
			const ps1Path = `${targetHookPath}.ps1`
	...
	for (const targetHookPath of targetHookPaths) {
		await fs.mkdir(path.dirname(targetHookPath), { recursive: true })
		const jsPath = `${targetHookPath}.js`
		const shellBridge = buildUnixNodeBridge(process.execPath, path.basename(jsPath))
		await fs.writeFile(jsPath, nodeScript)
		await fs.chmod(jsPath, 0o755)
		await fs.writeFile(targetHookPath, shellBridge)
...
function buildUnixNodeBridge(nodePath: string, jsFileName: string): string {
	const escapedNodePath = nodePath.replace(/'/g, "'\\''")
	const escapedJsFileName = jsFileName.replace(/'/g, "'\\''")
	...
	return [
		"#!/bin/sh",
		"set -eu",
		`script_path="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)/${escapedJsFileName}"`,
		'stdin_file="$(mktemp)"',
		'stdout_file="$(mktemp)"',
		'stderr_file="$(mktemp)"',
		'cleanup() { rm -f "$stdin_file" "$stdout_file" "$stderr_file"; }',
		"trap cleanup EXIT",
		'cat > "$stdin_file"',
		`'${escapedNodePath}' "$script_path" < "$stdin_file" > "$stdout_file" 2> "$stderr_file"`,
		"status=$?",
		'cat "$stdout_file"',
		'cat "$stderr_file" >&2',
		"exit $status",
 ...
 * // Hook is now available at tempDir/.tasktronautrules/hooks/PreToolUse
 */
