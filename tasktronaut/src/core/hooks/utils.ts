...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Tasktronaut", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".tasktronautrules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".tasktronautrules", "hooks")
}
