 ...
 * and workspace-specific hooks (from each workspace's .tasktronautrules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .tasktronautrules that they don't want to execute
...
export class HookFactory {
	 ...
	 * - Workspace hooks (from workspaceRoot/.tasktronautrules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(
		...
		// Workspace hooks are at: workspaceRoot/.tasktronautrules/hooks/
	 ...
	 * Workspace hooks are located in workspace .tasktronautrules/hooks/ directories
	 *
	 ...
	 * (from .tasktronautrules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .tasktronautrules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .tasktronautrules directory path to search
