...
import { coerceSupportedApiProvider, SUPPORTED_API_PROVIDER, type ApiProvider, type ModelInfo } from "@shared/api"
	...
	*/
	...
	async handleSignOut() {
		try {
			...
			const updatedConfig = {
				...apiConfiguration,
				planModeApiProvider: SUPPORTED_API_PROVIDER as ApiProvider,
				actModeApiProvider: SUPPORTED_API_PROVIDER as ApiProvider,
			}
	...
	async initTask(
		...
		await AgentConfigLoader.setWorkspacePath(cwd)
	...
	async handleAuthCallback(customToken: string, provider: string | null = null) {
		try {
			...
			if (planActSeparateModelsSetting) {
				...
				if (currentMode === "plan") {
					updatedConfig.planModeApiProvider = coerceSupportedApiProvider(updatedConfig.planModeApiProvider)
				} else {
					updatedConfig.actModeApiProvider = coerceSupportedApiProvider(updatedConfig.actModeApiProvider)
				}
			} else {
				// Update both modes to keep them in sync
				updatedConfig.planModeApiProvider = SUPPORTED_API_PROVIDER
				updatedConfig.actModeApiProvider = SUPPORTED_API_PROVIDER
			}
	...
	// MCP Marketplace
	// FORK MOD: External marketplace fetch disabled for ITAR/EAR compliance.
	// Only locally-configured MCP servers are allowed. No calls to cline.bot.
