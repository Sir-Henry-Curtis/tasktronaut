	...
	const filteredTaskSettings: Partial<Settings> = Object.fromEntries(
		Object.entries({
			...
			...(request.taskSettings?.planModeApiProvider !== undefined && {
				planModeApiProvider: coerceSupportedApiProvider(
					convertProtoToApiProvider(request.taskSettings.planModeApiProvider),
				),
			}),
			...(request.taskSettings?.actModeApiProvider !== undefined && {
				actModeApiProvider: coerceSupportedApiProvider(
					convertProtoToApiProvider(request.taskSettings.actModeApiProvider),
				),
			}),
