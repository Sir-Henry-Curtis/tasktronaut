...
import { ApiHandlerOptions, ApiProvider, coerceSupportedApiProvider } from "@/shared/api"
			...
			for (const [key, value] of Object.entries(protoOptions)) {
				if (maskOptionsFields.has(key)) {
					...
					if (key === "planModeApiProvider") {
						options.planModeApiProvider = coerceSupportedApiProvider(convertProtoToApiProvider(value))
					} else if (key === "actModeApiProvider") {
						options.actModeApiProvider = coerceSupportedApiProvider(convertProtoToApiProvider(value))
					} else {
