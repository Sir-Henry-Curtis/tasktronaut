...
import { sendGsdButtonClickedEvent } from "./core/controller/ui/subscribeToGsdButtonClicked"
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// FORK MOD: Inject government-approved API endpoint before any other init.
	// This overrides any previously stored provider config on every activation,
	// ensuring the approved endpoint is always used even if storage was tampered with.
	await injectApprovedApiConfig(context)
	...
	// Make workspace-local Tasktronaut shims available to terminal-driven tools.
	if (workspacePath) {
		const workspaceBin = path.join(workspacePath, ".tasktronaut", "bin")
		context.environmentVariableCollection.prepend("PATH", `${workspaceBin}${path.delimiter}`)
	}
	...
	*/
	...
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "tasktronaut.isDevMode", IS_DEV)
...
// FORK MOD: Force-write government-approved API config into secret storage on every
// activation so the approved endpoint is always used, regardless of prior stored config.
async function injectApprovedApiConfig(context: vscode.ExtensionContext): Promise<void> {
	try {
		const provider = process.env.APPROVED_API_PROVIDER
		const baseUrl = process.env.APPROVED_API_BASE_URL
		const modelId = process.env.APPROVED_MODEL_ID
		const apiKey = process.env.APPROVED_API_KEY

		if (provider) await context.secrets.store("apiProvider", provider)
		if (baseUrl) await context.secrets.store("apiBaseUrl", baseUrl)
		if (modelId) await context.secrets.store("apiModelId", modelId)
		if (apiKey) await context.secrets.store("apiKey", apiKey)

		if (provider || baseUrl || modelId) {
			Logger.info("[FORK] Injected approved API config from environment")
		}
	} catch (error) {
		Logger.warn(`[FORK] Failed to inject approved API config: ${error instanceof Error ? error.message : String(error)}`)
	}
}
